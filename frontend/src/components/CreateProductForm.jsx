...
const convertProductForForm = (product) => {
	if (!product) {
	...
	return {
		...
		options: {
			...
			sizes: Array.isArray(product?.options?.sizes)
...
const CreateProductForm = ({
	...
	const isEditing = Boolean(editProduct?._id);
	...
	useEffect(() => {
		if (isEditing) {
			setNewProduct(
				convertProductForForm(editProduct)
			);
		} else {
			...
			>
				...
				<div className='rounded-xl border border-gray-700 bg-gray-900 p-4'>
					...
					<p className='text-sm text-gray-400 mb-4'>
						Add exact combinations like Black /
						Small, Black / Medium or White /
						Small. Each combination can have its
						own stock, SKU, price and images.
					</p>
				...
				<div>
					...
					<label
						...
						className='cursor-pointer inline-flex items-center bg-gray-700 py-2 px-3 border border-gray-600 rounded-md text-sm font-medium text-gray-300 hover:bg-gray-600'
					>
				...
				<div className='flex flex-col sm:flex-row gap-3'>
					{isEditing && (
						<button
							...
							onClick={onCancelEdit}
							disabled={loading}
							className='sm:w-auto px-5 py-3 rounded-md text-sm font-medium text-gray-200 bg-gray-700 hover:bg-gray-600 disabled:opacity-50'
						>
							Cancel
						</button>
					)}
					...
					<button
						type='submit'
						className='flex-1 flex justify-center items-center py-3 px-4 rounded-md text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50'
						disabled={
							loading || !canSubmit
						}
					>
