...
const ProductCard = ({ product }) => {
	...
	const handleAddToCart = (e) => {
		...
		// ✅ Keep cart item shape consistent (so orders can store specifics later)
		// ProductCard has no selected color/size yet, so we store empty defaults.
		addToCart({
			...product,
			selectedOptions: {
				color: "",
	...
	// ✅ ZAR formatting (enterprise look)
	const formatZar = (n) =>
	...
	// ✅ Stock display (supports both: stockQuantity and total variant stock)
	...
	const stock = stockQuantity !== null ? stockQuantity : variantStockTotal;
	...
	const isLowStock =
		typeof stock === "number" && stock > 0 && stock <= lowStockThreshold;
		...
		>
			...
			<div className="mt-4 px-5 pb-5">
				...
				<button
					...
					className={`flex w-full items-center justify-center rounded-lg px-5 py-2.5 text-center text-sm font-medium
